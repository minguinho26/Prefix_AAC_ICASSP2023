...
# input으로 raw_data를 뽑아낼 때 쓰는 메소드
def _clotho_dataloader_preprocess(batch: MutableSequence[ndarray]) \
    ...
    input_tensor = cat([from_numpy(i[0]).float().unsqueeze(0) for i in batch])
...
def _clotho_collate_fn(batch: MutableSequence[ndarray]) \
    ...
    input_tensor = cat([
                        cat([zeros(max_input_t_steps - i[0].shape[0], input_features).float(),
